...
import {
  ...
  CheckCircle2, ImageIcon, X, Plus, Minus, Trash2, Search, ShoppingCart, ChevronDown, ChevronUp, Banknote, Smartphone
} from "lucide-react";
...
function AdminPOS({ onOrderCompleted, onCancel, employeeNumber }: { onOrderCompleted: () => void, onCancel: () => void, employeeNumber: string }) {
  ...
  const handleCheckout = async (method: 'cash' | 'upi') => {
    ...
    try {
      ...
      if (error && (
        ...
        // Fallback for when the employee_number or phone_number column hasn't been added to the database yet
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const checkUser = async () => {
      ...
      if (isAdmin) {
        // Allow admins to view the employee page for testing without redirecting them.
        // router.push("/admin");
        // return;
      }
      ...
      channel = supabase
        .channel(`employee-realtime-${Date.now()}`)
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "orders" },
          (payload) => {
            console.log("[Employee Realtime] Orders change:", payload.eventType);
            fetchOrders();
          }
        )
        .on(
          ...
          (payload) => {
            console.log("[Employee Realtime] Expenses change:", payload.eventType);
            fetchExpenses();
          }
        )
        .subscribe((status) => {
          console.log("[Employee Realtime] Channel status:", status);
        });
    };
    checkUser();
  ...
  const handleLogout = async () => {
    const sessionId = localStorage.getItem('employee_session_id');
    if (sessionId) {
      await supabase.from('employee_activity').update({ logout_time: new Date().toISOString() }).eq('id', sessionId);
      localStorage.removeItem('employee_session_id');
    }
    await supabase.auth.signOut();
    router.push("/login?role=employee");
  };
  ...
  const isEmployee = adminEmail.endsWith("@employee.meltedstories.com");
  const tabs = isEmployee ? allTabs.filter(t => t.id !== "expenses") : allTabs;
  ...
  return (
    <main className="min-h-screen bg-luxury pb-24">
      {/* Payment Proof Modal */}
      {proofModal && (
      ...
      <header className="sticky top-0 z-50 header-cream">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <span className="text-lg font-serif text-gradient-gold tracking-[0.2em] uppercase">{isEmployee ? 'Employee Panel' : 'Admin Panel'}</span>
      ...
      <div className={`mx-auto px-4 pt-5 pb-24 ${activeTab === 'pos' ? 'max-w-5xl' : 'max-w-2xl'}`}>
        ...
        {activeTab === "pos" && (
          <AdminPOS
            ...
            employeeNumber={adminEmail.replace('@employee.meltedstories.com', '')}
          />
        )}
        ...
        {activeTab === "profile" && (
          <div className="animate-fadeUp">
            <div className="mb-6">
              <h2 className="font-serif text-2xl text-choco">{isEmployee ? 'Employee Profile' : 'Admin Profile'}</h2>
            ...
            <div className="card-choco rounded-3xl p-8 mb-6 text-center border border-amber-500/20 shadow-2xl relative overflow-hidden">
              ...
              <p className="text-amber-200/50 text-xs mt-1 uppercase tracking-widest font-black">{isEmployee ? 'Lounge Employee' : 'Lounge Administrator'}</p>
            </div>
            ...
            <div className="card-choco rounded-2xl p-5 mb-6 border border-amber-900/30">
              <div className="flex justify-between items-center py-3.5 border-b border-amber-900/30">
                ...
                <span className="text-amber-100 text-sm font-semibold">{isEmployee ? 'Staff Member' : 'Owner / Admin'}</span>
              </div>
              <div className="flex justify-between items-center py-3.5 border-b border-amber-900/30">
